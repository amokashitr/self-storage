...
def units(amenities)
  units = []
  for i in 0..9
    unit = Unit.new(length: (5 * rand(1..4)), breadth: (5 * rand(1..4)), quantity: rand(1200..2000), rate: rand(300..4000), offer: '15% Off and First Month Free')
...
CSV.foreach(file_path, headers: true) do |row|
  ...
  storage_unit = StorageUnit.new(row.select { |k, v| STORAGE_UNIT_COLUMNS.include?(k) })
  storage_unit.rating = [0,1,2,3,4,5].sort{ rand() - 0.5 }.take(1).last
  address_attributes = { street: row[:street]}
  address_attributes = address_attributes.merge(parse_address(row, row[:address]))
  storage_unit.address = storage_unit.build_address(address_attributes)
  storage_unit.amenities = amenities.sample(4)
  storage_unit.units = units(amenities)
  storage_unit.save
end


# Address.import storage_units_rows, recursive: true
